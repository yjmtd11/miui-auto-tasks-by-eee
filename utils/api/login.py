...
class Login:
    ...
    async def login(
        ...
        try:
            ...
            if repo_owner not in [None, "0-8-4"]:
                return False
            daily_tasks = await BaseSign(self.account).check_daily_tasks(nolog=True)
            if self.cookies != {} and daily_tasks != []:
    ...
    async def get_cookies_by_passtk(self, user_id: str, pass_token: str):
        ...
        try:
            ...
            response = await get(url, cookies=cookies, headers=headers, allow_redirects=False)
            url = response.headers.get("location")
